...
#define BAR_SEL_COLOR [UIColor colorWithRed:98.0f/255.0f green:102.0f/255.0f blue:216.0f/255.0f alpha:0.8]
...
- (void)initialize {
    ...
    //Set the acutal date
    if (self.initialDate) {
        _selectedDate = self.initialDate;
    } else {
...
//    [saveArea addSubview:_saveButton];
    ...
    //Set the time to now
    if (self.initialDate) {
        NSString *strTime = [self stringFromDate:self.initialDate withFormat:@"hh:mm a"];
